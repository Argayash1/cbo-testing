...
import { CardSlider, CTA, SectionSubtitle, SectionTitle } from '../../components';
...
export const HowItWillGo = () => {
  ...
  const photoListItems = photos.map((photo, index) => (
    <li key={index} className={styles.photoListItem}>
      <img src={photo.image} alt={photo.altText} className={styles.photo} loading='lazy' />
      <p className={styles.textOnPhoto}>{hoItWillGoItems[index]}</p>
    </li>
  ));
  const initialSliderOffset = screenWidth > 531 ? 50 : 50.7;
  const offset = screenWidth > 986 ? 707 : screenWidth <= 986 && screenWidth > 531 ? 507 : 285;
  const nextButtonDisabled = photos.length - switchCount <= 1;

  return (
    <section className={styles.root}>
      <div className={styles.container}>
        <SectionTitle titleText='Как все будет проходить?' place='how-will-it-go' />
        <SectionSubtitle text='Предметный разговор без купюр:' place='how-it-will-go' />
        <CardSlider
