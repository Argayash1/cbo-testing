...
export const FAQAccordionCard = ({ question, answer, onClick, isOpen }: FAQAccordionCardProps) => {
  ...
  return (
    <article className={styles.root}>
      ...
      <div
        ...
        style={{
          height: isOpen ? `${answerRef.current?.scrollHeight && answerRef.current.scrollHeight}px` : '0',
        }}
      >
        <p className={styles.answer} ref={answerRef}>
          {screenWidth <= 986 && screenWidth > 531 && answer.startsWith('Текущие') ? secondAnswerOnTablet : answer}
        </p>
      </div>
    </article>
  );
