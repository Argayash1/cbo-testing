...
export const ActivityCard = ({ title, subtitle, ideasList, addition, summaryList, icon }: ActivityCardProps) => {
  ...
  const subtitleTextContent = isThirdCard && screenWidth <= 986 ? subtitleWithNonBreakingSpace : subtitle;
  ...
  return (
    ...
    >
      ...
      {subtitle && <p className={styles.subtitle}>{subtitleTextContent && replaceStar(subtitleTextContent)}</p>}
      {ideasList && <ul className={styles.ideasList}>{ideasListItems}</ul>}
      {addition && <p className={styles.addition}>{additionContent}</p>}
      {summaryList && <ul className={styles.summaryList}>{summaryListItems}</ul>}
      {!subtitle && (
        <>
          <CTA path='#' linkText='Скачать программу' type='practice-activity' linkType='download' />
